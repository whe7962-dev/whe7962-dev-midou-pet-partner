/**
 * Native lifecycle adapter for React Bits AeroShards.
 * Original Copyright (c) 2026 David Haz.
 * Source: https://github.com/DavidHDev/react-bits/tree/main/src/content/Backgrounds/AeroShards
 * License: ./licenses/REACT-BITS-LICENSE.txt (MIT with Commons Clause).
 * The original WGSL shaders and rendering graph are retained.
 */
import { draw, effect, frame, init, sampler, surface, target, uniforms } from './vendor/vgpu.js';
...
/**
 * Mount the supplied canvas; all visual options follow the upstream component.
 * ready resolves true after the first presented frame, false after failure/disposal.
 * setOptions returns false after disposal/failure. No global styles are modified.
 */
export function mountAeroShards(root, initialOptions = {}) {
  if (!root || typeof root.querySelector !== 'function' || !root.dataset) {
    throw new TypeError('AeroShards requires a root element.');
  }
  const canvas = root.querySelector('canvas.aero-shards__canvas');
  if (!canvas) throw new TypeError('AeroShards requires canvas.aero-shards__canvas.');
  const validateOptions = value => {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      throw new TypeError('AeroShards options must be an object.');
    }
    for (const [key, val] of Object.entries(value)) {
      if (typeof val === 'number' && !Number.isFinite(val)) {
        throw new TypeError('AeroShards option ' + key + ' must be finite.');
      }
    }
    const numericOptions = ['scale', 'spread', 'depth', 'speed', 'spin', 'density', 'shardSize',
      'stretch', 'turbulence', 'glow', 'edgeSoftness', 'bloom', 'grain', 'chromaticAberration',
      'transitionDuration', 'interactionRadius', 'interactionStrength', 'rippleIntensity'];
    for (const key of numericOptions) {
      if (key in value && (typeof value[key] !== 'number' || !Number.isFinite(value[key]))) {
        throw new TypeError('AeroShards option ' + key + ' must be a finite number.');
      }
    }
    if (value.onError != null && typeof value.onError !== 'function') {
      throw new TypeError('AeroShards onError must be a function.');
    }
  };
  validateOptions(initialOptions);
  let options = { ...initialOptions };
  const onErrorRef = { current: options.onError };
  const settingsRef = { current: resolveSettings(options) };
  const wakeRef = { current: () => {} };
  const pointerRef = { current: {
    raw: [0.5, 0.5],
    position: [0.5, 0.5],
    velocity: [0, 0],
    active: 0,
  ...
  const setReady = value => {
    root.dataset.ready = String(value);
    if (value) resolveReady(true);
  };
  setReady(false);
  ...
  const reportFailure = error => {
    ...
    if (onErrorRef.current) {
      try { onErrorRef.current(resolved); }
      catch (callbackError) { console.error('AeroShards error callback failed:', callbackError); }
    }
  };
  ...
  const inputDisabled = () => disposed || runtimeFailed || !visible || document.hidden ||
    settingsRef.current.paused || reduceMotion.matches || settingsRef.current.speed <= 0.0001;
  ...
  const handlePointerMove = event => {
    ...
    if (!event.isPrimary || inputDisabled() || settings.interaction === INTERACTIONS.none) return;
    ...
    // Native scrolling always wins over a decorative hold gesture.
    if (event.pointerType === 'touch' && touchScrolling) return;
  ...
  const handlePointerDown = event => {
    const settings = settingsRef.current;
    if (!event.isPrimary || event.button !== 0 || inputDisabled() || settings.interaction === INTERACTIONS.none) return;
  ...
  const markBoundsDirty = () => {
    boundsDirty = true;
    if (touchStart || touchScrolling) {
      touchScrolling = true;
      resetInteraction();
      if (!inputDisabled()) wakeRenderer();
    }
  };
  ...
  const handleVisibilityChange = () => {
    ...
    if (!document.hidden && visible) wakeRenderer();
  };
  ...
  void Promise.resolve().then(async () => {
    ...
    try {
      ...
      if (!navigator.gpu || typeof navigator.gpu.getPreferredCanvasFormat !== 'function') {
        throw new Error('WebGPU is unavailable. A static fallback is required.');
      }
      const resolvedQuality = resolveQuality(canvas);
      const preset = QUALITY_PRESETS[resolvedQuality] || QUALITY_PRESETS.medium;
      const initializedGpu = await init({ powerPreference: 'low-power' });
      if (disposed || runtimeFailed) return initializedGpu.dispose();
      gpu = initializedGpu;
      ...
      const renderFrame = currentFrame => {
        ...
        if (firstFrame) {
          ...
          readyFrameId = requestAnimationFrame(() => {
            readyFrameId = 0;
            if (!disposed && !runtimeFailed) setReady(true);
          });
        }
      };
  ...
  const setOptions = partial => {
    if (disposed || runtimeFailed) return false;
    validateOptions(partial);
    options = { ...options, ...partial };
    settingsRef.current = resolveSettings(options);
    onErrorRef.current = options.onError;
    if (settingsRef.current.paused || reduceMotion.matches) {
      resetInteraction();
      cancelScheduledFrames();
    }
    resumePending = true;
  ...
  return Object.freeze({ setOptions, dispose, ready });
}
