...
首屏使用原创角色素材演绎自动微距拉远、轻柔悬浮与缓慢视角跟随；无需播放、拖动或选择章节。
...
- 尊重 `prefers-reduced-motion`，使用静态角色、完整文字，关闭眨眼、呼吸、自动轮播、视差和指针姿态变化。
...
测试覆盖首屏与碎片背景的生命周期、后台暂停、减少动态偏好、兼容降级、触屏滚动取消聚拢及音效开关；静态结构检查覆盖品牌、首屏功能入口、七张照片、资源引用、唯一 ID 与内容背景布局。原 28 秒时间轴源码与回归测试仍保留，但不再由首页加载。测试验证脚本逻辑与结构，不替代浏览器视觉、眼部蒙版对齐和真实触屏检查。
...
咪Dou、背景动效与内容卡是品牌或功能概念展示，不代表实体传感器或已验证的诊疗能力。页面里的 App UI 为功能示意，没有连接问诊后端或实际安装包。下载按钮会展示明确的待开放提示。
